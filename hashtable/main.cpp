...
public:
    ...
    int FindMax()
    {
        int maximum=0, hMax=0;
        for (int i=0;i<size_all_non_nullptr;i++)
        {
            std::cout<<1<<std::endl;
            if (maximum<arr[i]->repetitions && arr[i]->state==true) {maximum=arr[i]->repetitions;hMax=i;}
        }
        return hMax;
    ...
    bool Add(const T& value, const THash1& hash1 = THash1(),const THash2& hash2 = THash2())
    {
        ...
        {
            if (arr[h1]->value == value && arr[h1]->state)
            {arr[h1]->repetitions=arr[h1]->repetitions+1;return false;} // такой элемент уже есть, а значит его нельзя вставлять повторно
        ...
        else
        {
            ...
            arr[first_deleted]->repetitions = 1;
...
int main() {
    ...
    while(a[a.length()-1]!='0' || a=="")
    {
        std::cin>>a;
        table->Add(a);
        //if (table->Find(a)==true) std::cout<<a<<std::endl;
    }

    std::cout<<table->FindMax()<<std::endl;
    std::cout<<a<<std::endl;
    return 0;
}
